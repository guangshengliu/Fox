...
unsigned long ioapic_rte_read(unsigned char index)
{
	...
	*ioapic_map.virtual_index_address = index + 1;
	//	Execution stream of serialized processors
	io_mfence();
...
/*
*	Physical address 0xfec00000 map to linear address 0xffff8000fec00000
*/
...
void Local_APIC_init()
{
	...
	//	Enable SVR[8]:open Local APIC
	//	Enable SVR[12]:no support broadcast EOI messages
...
void IOAPIC_init()
{
	...
	*ioapic_map.virtual_index_address = 0x01;
	...
	//	Initialize RTE entry
	for(i = 0x10;i < 0x40;i += 2)
		ioapic_rte_write(i,0x10020 + ((i - 0x10) >> 1));
	//	interrupt vector number ：0X21	Delivery mode ： physical mode
	//	Destination field ： APIC ID is 0
...
void APIC_IOAPIC_init()
{
	...
	//get RCBA address
	io_out32(0xcf8,0x8000f8f0);		//通过间接寻址方式索引到RCBA寄存器
	x = io_in32(0xcfc);				//通过间接寻址的方式所引导RCBA寄存器
