...
unsigned long page_init(struct Page * page,unsigned long flags)
{
	...
	// 当前页面结构属性或参数flag中含有引用属性(PG_Referenced)或共享属性(PG_K_Share_To_U)
	// 只增加struct page结构体的引用计数和struct zone结构体的页面被引用计数
...
void init_memory()
{
	...
	// 将页目录中的前10个页表项清零，消除一致性(TLB与PML4页表)页表映射
	//for(i = 0;i < 10;i++)
...
/*
	...
	zone_select: 从DMA区域空间，已映射页表区域空间或者未映射页表区域空间里，一次申请64个连续的物理页
...
struct Page * alloc_pages(int zone_select,int number,unsigned long page_flags)
{
	...
	// 从目标内存区域空间的起始内存页结构开始逐一遍历，直到内存区域空间的结尾
